...
class ScreenUtil {
  ...
  /// Whether the device supports split-view layout (tablet/desktop)
  static bool supportsSplitView(BuildContext context) {
    return width(context) >= 720;
  }

  /// Chat-specific: get the conversation list width in split view
  static double getConversationListWidth(BuildContext context) {
    final screenWidth = width(context);
    if (isMobileWidth(screenWidth)) return screenWidth;
